...
def filter_big_scc(g, edges_to_be_removed):
    ...
    # Deprecation warning: nx.strongly_connected_component_subgraphs is deprecated
    # sub_graphs = filter(lambda scc: scc.number_of_nodes() >=
    #                     2, nx.strongly_connected_component_subgraphs(g))
    comp_subgraphs = [g.subgraph(c) for c in nx.strongly_connected_components(g)]
    sub_graphs = filter(lambda scc: scc.number_of_nodes() >= 2, comp_subgraphs)
    return sub_graphs
...
def get_big_sccs(g):
    ...
    edges_to_be_removed = []
    big_sccs = []
    # for sub in nx.strongly_connected_component_subgraphs(g):  Deprecated !!
